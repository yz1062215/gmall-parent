...
public class OrderStockDeduceListener {
    ...
    public void stockDeduceListener(Message message, Channel channel) throws IOException {
        ...
        try {
            ...
            //查询订单
            OrderInfo orderInfo = orderInfoService.getById(orderId);//查询所有库
            ProcessStatus status = null;
            switch (msg.getStatus()){
                case "DEDUCTED": status = ProcessStatus.WAITING_DELEVER;//库存扣减成功  等待发货
                break;
                case "OUT_OF_STOCK": status = ProcessStatus.STOCK_OVER_EXCEPTION;//超卖  异常  其他仓库调货
                break;
            ...
            String uniKey = SysRedisConst.MQ_RETRY + "stock:order:deduce:" + orderId;
            rabbitService.retryConsumerMsg(10L,uniKey,tag,channel);
        }
